...
- 右键打开“键鼠统计”：可在“今日 / 累计”间切换，用全尺寸键盘热力图查看每个按键的次数（左右 Ctrl、Shift、Alt 及主键盘/数字键盘 Enter 分开统计），并查看鼠标左键、右键和滚轮点击次数。
...
位置、尺寸、空闲动作和自定义对话保存在 exe 同目录的 `豆豆桌宠配置.ini`。键鼠统计以稀疏数据保存在同目录的 `豆豆桌宠输入统计.json`，持续输入时仅更新内存，空闲或退出时才写盘；只记录按键标识和次数，不记录输入内容。只有开机自动启动功能使用当前用户的 Windows `Run` 注册表项。
